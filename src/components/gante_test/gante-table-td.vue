<template>
  <div>
    <div
      ...
      :style="{width:th.width ? th.width+'px' : '80px',textIndent:th.shrink ? (td_data.level-1)*10+'px' : null,fontWeight:(td_data.level ==1 && th.shrink && !th.weight) ?'bold':'normal'}"
    >
      <span style="width: 15px;display: inline-block;text-indent: 0" v-if="th.shrink">
        <span
          v-show="td_data.children"
          style="cursor: pointer;"
          @click.stop="opens(td_data.gunter_id)"
        >
          <span v-show="!td_data.open">
            <img style="width: 13px;" src="../../assets/add.png" alt />
          </span>
          <span v-show="td_data.open">
            <img style="width: 13px;" src="../../assets/reduce.png" alt />
          </span>
        </span>
      </span>
      <span
        :title="key_value"
        :class="{'state_color1 cell-value': td_data.params.state == 1,
                  'state_color2 cell-value': td_data.params.state == 2,
                  'state_color3 cell-value': td_data.params.state == 3,
                  'state_color4 cell-value': td_data.params.state == 4}"
      >{{td_data.params[key_value]}}</span>
    </div>
    <input
      class="gante_edit_input_liu"
      @click="change_calendar($refs.cells,td_data.params[key_value])"
      :readonly="th.chooseTime"
      ref="gante_input"
      v-if="edit"
      @blur="td_blur(td_data.gunter_id)"
      type="text"
